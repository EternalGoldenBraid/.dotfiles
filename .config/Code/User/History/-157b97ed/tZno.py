...
class AudioPlayer_parallel:
    def __init__(self):
        self.p = pyaudio.PyAudio()

    def beep(self, frequency, duration, volume=1.0):
        def play_beep():
            # Generate the beep samples
            num_samples = int(self.p.get_sample_rate() * duration)
            samples = (np.sin(2 * np.pi * np.arange(num_samples) * frequency / self.p.get_sample_rate())).astype(np.float32)

            # Open a streaming stream
            stream = self.p.open(format=pyaudio.paFloat32,
                                 channels=1,
                                 rate=self.p.get_sample_rate(),
                                 output=True)
