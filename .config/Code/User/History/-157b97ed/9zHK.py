...
class AudioPlayer:
    ...
    def play_beep(self, frequency, duration=0.1):
        samples = np.array([np.sin(2.0 * np.pi * frequency * x / self.sample_rate) for x in range(int(self.sample_rate * duration))]).astype(np.float32)
...
if __name__ == "__main__":
    ...
    audio_player.play_beep(440)  # Play a 440 Hz beep for 0.1 seconds (default duration)
