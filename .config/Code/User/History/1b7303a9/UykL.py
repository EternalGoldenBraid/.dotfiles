...
from torch.random import default_generator as rng_gen
...
from src.data.vaalikone_yle_2019_dataset import MLMVaaliKoneDataset
...
def train_mlm(cfg: DictConfig) -> None:
    RNG = rng_gen.manual_seed(cfg.seed)
    ...
    model_conf = cfg.model.language_encoders.perttu
    ...
    # )
    dataset = MLMVaaliKoneDataset(
        root=hydra.utils.to_absolute_path(cfg.data.dataset.root),
        filename='yle_vaalikone_2019_no_empty_answers.csv',
        config=cfg, tokenizer=tokenizer
    )
    df = dataset.df
    ...
    train_loader = DataLoader(train_dataset, batch_size=model_conf.batch_size,
                            shuffle=True, collate_fn=data_collator, generator=RNG)
    val_loader = DataLoader(val_dataset, batch_size=model_conf.batch_size,
                            shuffle=False, collate_fn=data_collator, generator=RNG)
    test_loader = DataLoader(test_dataset, batch_size=model_conf.batch_size,
                            shuffle=False, collate_fn=data_collator, generator=RNG)
