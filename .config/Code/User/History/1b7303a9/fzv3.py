...
def train_mlm(cfg: DictConfig) -> None:
    ...
    val_loader = DataLoader(val_dataset, batch_size=model_conf.batch_size, shuffle=True, collate_fn=data_collator)
    val_metrics = evaluate_mlm(cfg=cfg, model=model, dataloader=val_loader, device=device, max_samples=max_samples)
