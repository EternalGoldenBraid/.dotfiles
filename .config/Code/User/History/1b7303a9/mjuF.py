...
def train_mlm(cfg: DictConfig) -> None:
    ...
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, collate_fn=data_collator)
    ...
    test_metrics = evaluate_mlm(cfg=cfg, model=model, dataloader=test_loader, device=device)
    print(f"Test metrics: {test_metrics}")
    # val_metrics = evaluate_mlm(cfg=cfg, model=model, dataloader=val_loader, device=device)
