...
class Ribosome:
    ...
    def interact_with(self, other_ribosome, 
        ...
        if (self.position - self.old_position) * (other_ribosome.position - other_ribosome.old_position) > 0:
            ...
            frequency = (self.base_frequency+other_ribosome.base_frequency)/2
...
def generate_circle_positions(layer_nodes, inter_layer_dist=50, intra_layer_dist=50):
    positions = []
    num_layers = len(layer_nodes)
    
    for layer_idx, num_nodes in enumerate(layer_nodes):
        layer_x = layer_idx * inter_layer_dist
        layer_y_offset = -intra_layer_dist * (num_nodes - 1) / 2
        
        for node_idx in range(num_nodes):
            node_y = layer_y_offset + node_idx * intra_layer_dist
            positions.append(np.array([layer_x, node_y]))
