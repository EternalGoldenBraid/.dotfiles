...
class CircleRNA:
    def __init__(self, ax, center, cmap, radius: float,
        ...
        for i in range(n_ribo):
            self.ribosomes.append(Ribosome(circle=self, position=0, base_frequency=self.base_frequency, radius=self.radius,
    ...
    def interact_with(self, other_circle, strength=0.001, action='attract'):
        ...
        if np.linalg.norm(self.center - other_circle.center) <= 2*self.radius:
            return
...
class Ribosome:
    ...
    def get_location(self):
        ...
        # print(self.circle.center-self.circle.circle.center)
        # circle_center: np.array = self.circle.center
        circle_center: np.array = self.circle.circle.center
        return circle_center[0] + self.radius*(np.cos(angle)), circle_center[1] + self.radius*(np.sin(angle))
    ...
    def interact_with(self, other_ribosome, 
        ...
        if (self.position - self.old_position) * (other_ribosome.position - other_ribosome.old_position) > 0:
            other_ribosome.update_position(
                # other_ribosome.position*(gamma + 1.) + gamma*self.old_position
                # other_ribosome.position + gamma*(self.position - self.old_position)
                # other_ribosome.position + gamma
            ...
            other_ribosome.update_position(
                # other_ribosome.position*(gamma - 1.01) + gamma*self.old_position)
                # other_ribosome.position*(gamma - 1.) + gamma*self.old_position
                # other_ribosome.position - gamma
                other_ribosome.position - gamma*difference_norm
                )
