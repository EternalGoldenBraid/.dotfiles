...
#     def __init__(self, config: DictConfig):
        # super(BertEncoder, self).__init__()
class BertEncoder():
    def __init__(self, config: DictConfig, max_n_questions, device='cpu'):
        # super(BertEncoder, self).__init__()
        ...
        self.outputs: torch.tensor = torch.zeros((max_n_questions, self.nfeats), dtype=torch.float32, device=self.device)
    ...
    def encode(self, input_ids: torch.tensor, attention_mask: torch.tensor,
        ...
        if self.config.data_parallel != True:
            output = self.bert_model(input_ids=input_ids,
                attention_mask=attention_mask).pooler_output
        elif self.config.data_parallel == True:
    ...
    def encode2(self, input_ids: torch.tensor, attention_mask: torch.tensor,
                token_type_ids: torch.tensor):
        ...
        if len(input_ids.shape) != 3:
            raise ValueError("input_ids.shape should be (batch_size, n_questions, n_tokens)")
        elif len(input_ids.shape) == 3 and input_ids.shape[0] != 1:
            raise NotImplementedError("Batch size > 1 not implemented yet.")

        # Take average of all questions for each participant
        if self.config.data_parallel != True:
            output = self.bert_model(input_ids=input_ids[0],
                attention_mask=attention_mask[0]).pooler_output
        elif self.config.data_parallel == True:
            input_ids = input_ids.permute(1,0,2)
            output = self.bert_model(input_ids=input_ids,
                attention_mask=attention_mask).pooler_output
            
        feature = torch.mean(output, dim=0)
    ...
    def bert_step(self, config, dataloader, features=None):
        ...
        for participant in tqdm(dataloader, desc='Participant', leave=False):
            ...
            if n_questions > config.bert.batch_size: 
                ...
                self.outputs: torch.tensor = torch.zeros((n_questions, self.nfeats), dtype=torch.float32, device=self.device)
                ...
                for batch_idx, (i_ids, i_mask, t_t_id) in enumerate(zip(input_ids_batch, attention_mask_batch, token_type_ids_batch)):
                    ...
                    outputs[start:end] = self.encode(i_ids, i_mask, t_t_id)
                    
                features[participant_idx] = outputs.mean(dim=0)
            else:
                # features[participant_idx] = self.encode(model3, input_ids, attention_mask,
                # features[participant_idx] = model3(input_ids, attention_mask,
                features[participant_idx] = self.encode(input_ids, attention_mask,
                                            token_type_ids).mean(dim=0)
                                            # token_type_ids).cuda(config.slaps_gpu)
