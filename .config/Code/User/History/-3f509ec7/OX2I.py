...
from src.slaps.layers import Diag
from src.slaps.utils import *
...
class MLP_generator(nn.Module):
    def __init__(self, nlayers, isize, hsize, osize, mlp_epochs, k, knn_metric, non_linearity, i, sparse, mlp_act, features):
    ...
    def mlp_knn_init(self):
        if self.input_dim == self.output_dim:
            ...
            labels = torch.from_numpy(nearest_neighbors(self.features.cpu(), self.k, self.knn_metric))

            for epoch in range(1, self.mlp_epochs):
                self.train()
                logits = self.forward(features=self.features)
