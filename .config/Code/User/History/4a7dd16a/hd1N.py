...
class EventFeatureDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for _, row in tqdm(self.df.iterrows(), total=len(self.df)):
            ...
            input_ids = story_encoder.tokenizer.encode(row['story'], return_tensors='pt',
                    max_length=self.config.model.language_encoders.bert.max_seq_length,
                    truncation=True, padding='max_length')

            with torch.no_grad():
                # embeddings = bert_encoder.model(input_ids)[0][:, 0, :]
                embeddings = bert_encoder.model(input_ids)

                decoded = bert_encoder.tokenizer.decode(input_ids[0])
                ...
                assert story == bert_encoder.tokenizer.decode(input_ids[0])
...
class BertEncoder(object):
    def __init__(self, config):
        self.config = config
        self.tokenizer = AutoTokenizer.from_pretrained(config.model.language_encoders.bert.model_name)
        self.model = AutoModel.from_pretrained(config.model.language_encoders.bert.model_name)

        for n, p in self.model.named_parameters():
            p.requires_grad = False

    def __call__(self, data):
        input_ids = self.tokenizer.encode(data.story, return_tensors='pt',
                            max_length=self.config.model.language_encoders.bert.max_seq_length,
                            truncation=True, padding='max_length')
        with torch.no_grad():
...
class MiniLMEncoder(object):
    def __init__(self, config):
        self.config = config
        self.tokenizer = AutoTokenizer.from_pretrained(config.model.language_encoders.minilm.model_name)
        self.model = AutoModel.from_pretrained(config.model.language_encoders.minilm.model_name)
    ...
    def __call__(self, data):
        encoded_input = self.tokenizer(data.story, return_tensors='pt',
