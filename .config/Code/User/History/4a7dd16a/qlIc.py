    ...
    def process(self):
        ...
        for _, row in tqdm(self.df.iterrows(), total=len(self.df)):
            ...
            with torch.no_grad():
                ...
                truncated = is_truncated_precise(row['story'], story_encoder.tokenizer,
                                                story_encoder.max_seq_length)
                ...
                # assert story == decoded or abs(len(story) - len(decoded)) < 20, f"""
                #     {story}
                #     !=
                #     {decoded}
                #     """
...
class BertEncoder(LanguageEncoder):
    def __init__(self, config):
        ...
        self.max_seq_length = config.model.language_encoders.bert.max_seq_length
...
class MiniLMEncoder(LanguageEncoder):
    def __init__(self, config):
        ...
        self.max_seq_length=self.config.model.language_encoders.minilm.max_seq_length
