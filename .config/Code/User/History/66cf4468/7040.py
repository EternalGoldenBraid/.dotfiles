...
from src.utils.utils import EarlyStopper, evaluate
...
class Experiment:
    ...
    def train_end_to_end_mlp(self, config):
        ...
        self.model_save_path = Path(config.paths.outputs_dir, config.model.slaps_save_dir, "best_model__mlp_clf.pt")
        ...
        for trial in range(args.ntrials):
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if epoch % args.log_train_interval == 0 or epoch == 1:
                ...
                with torch.no_grad():
                    ...
                    if val_accu > best_val_accu or val_loss < best_val_loss:
                        ...
                        evaluate(model=model, dataset=dataset, cfg=config)
    ...
    def train_end_to_end(self, config):
        ...
        self.model_save_path = Path(config.paths.outputs_dir, config.model.slaps_save_dir, "best_model_clf.pt")
        self.model_save_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        for trial in range(args.ntrials):
            ...
            for epoch in range(1, args.epochs_adj + 1):
                ...
                if epoch >= args.epochs_adj_warmup and epoch % args.log_train_interval == 0:
                    with torch.no_grad():
                        model1.eval()
                        model2.eval()
                        ...
                        evaluate(model=model2, dataset=dataset, cfg=config, adj=Adj)

                        # if val_accu > best_val_accu:
                        if val_loss < best_val_loss:
                            
                            # Save best model2 to disk
                            torch.save(model2.state_dict(), self.model_save_path)
                            # evaluate(model=model2, dataset=dataset, cfg=config, adj=Adj)
