...
# from transformers import get_linear_schedule_with_warmup, create_optimizer

from src.utils.utils import evaluate_mlm

import wandb

def train(model, train_dataloader, val_dataloader, config, device, model_save_path, dataset, collator):
    ...
    for epoch in range(config.training.epochs):
        ...
        max_samples = 3
        val_metrics = evaluate_mlm(cfg=config, model=model, dataset=dataset,
        ...
        if val_loss < best_val_loss:
            ...
            model.save_pretrained(model_save_path.parent)
            model.save_pretrained('/home/nicklas/VaaliPerttu/checkpoints/mlm')
