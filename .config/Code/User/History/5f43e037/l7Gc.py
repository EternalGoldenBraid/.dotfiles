...
class WatchManager:
    def __init__(self):
        ...
        ### Added by Nicklas (ix, iy, iz, r)
        self.quaternion: Tuple[float, float, float, float] = (0, 0, 0, 1)
    ...
    async def _detection_callback(self, device, advertisement_data):
        ...
        if SERVICE_UUID in advertisement_data.service_uuids:
            ...
            print(f"Found {name}")
            try:
                await self._do_connect(self.client, device)
    ...
    def on_quat(self, quaternion):
        self.quaternion:Tuple[float, float, float, float] = quaternion
