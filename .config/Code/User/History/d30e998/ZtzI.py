...
from utils import CircleRNA, Ribosome
...
class RiboVisualizer:
    def __init__(self, config, initial_positions: np.array, cmap, base_frequencies: List[int], radius: float = 2.):
        ...
        for idx, pos in enumerate(initial_positions):
            center = np.array([radius*idx, 2*radius*sin(radius*idx)])
            # center = f * self.index, 5* f * cos(f * self.index)
            # center = (2*self.radius * self.index), (2*self.radius * self.index)
            # center = (2*self.radius * self.index), exp(2*self.radius * self.index/100)
            # center = 20*f*cos(f*self.index), 20*f*sin(f*self.index)
        ...
        for i in range(len(ribosomes)):
            ...
            for j in range(i + 1, len(ribosomes)):
                ...
                if abs(ribosome1.angle - ribosome2.angle) <= angle_tolerance:
                    ...
                    # ribosome1.circle.interact_with(ribosome2.circle)
    ...
    def update(self, pos: np.array, cmap: mpl.colors.ListedColormap, path: Path,
        ...
        new_lines = self.draw_lines_between_same_angle_ribosomes(self.ribosomes, angle_tolerance=0.1)
...
if __name__ == '__main__':
    ...
    initial_positions = np.array([pi/n_circ * i  for i in range(n_circ)])
    # initial_positions = np.zeros(n_circ)
    # Random initials
    # initial_positions = np.random.random(n_circ) * 2 * pi
    vis = RiboVisualizer(None, initial_positions, cmap=cmap, radius=10*n_circ, base_frequencies=base_frequencies)
