...
class RiboVisualizer:
    def __init__(self, config, initial_positions: np.array, cmap, base_frequencies: List[int], n_circ: int, n_ribo:int, radius: float = 2.):
        ...
        for idx in range(n_circ):
            # center = np.array([20*radius*cos(20*pi/(idx+1)), 20*radius*sin(20*pi/(idx+1))])
            # center = np.array([8*radius*(idx+1), 10*radius*sin((idx+1))])
            center = np.array([10*radius*cos((idx+1)), 10*radius*sin((idx+1))])
            # center = np.array([20*radius*(idx+1), 20*radius*(idx+1)])
            circle = CircleRNA(ax=self.ax, center=center, cmap=cmap, radius=radius, n_ribo=n_ribo, 
        ...
        layer_nodes = [n_0, n_1, n_2, ..., n_n]  # Specify the number of nodes in each layer
        circle_positions = generate_circle_positions(layer_nodes)
                    ...
                    for ribosome2 in circle2.ribosomes:
                        ...
                        if np.abs(ribosome1.angle - ribosome2.angle) <= angle_tolerance:
                            ...
                            if self.adjacency_matrix[i, j]:
                                ...
                                line = self.ax.plot([loc1[0], loc2[0]], [loc1[1], loc2[1]], 'w-', linewidth=similarity, alpha=np.clip(similarity,0.,1.))[0]
                                lines.append(line)

                                # Update the ribosomes' state based on their interaction
                                ribosome1.interact_with(ribosome2, distance=self.distance_matrix[i, j], similarity=similarity)
...
if __name__ == '__main__':
    ...
    def func(t, x):
        ...
        # x[0:x.shape[0]-1] = sin(t)
        # x = np.sin(t+np.exp(x))
        # x = np.sin(t+np.cos(x))
    
        # Some sick function that will produce really weird circles
        # sick_output = np.sin(t*x) + np.cos(t*x) + np.sin(t*x**2) + np.cos(t*x**2) + np.sin(t*x**3) + np.cos(t*x**3)
        # x = sick_output % (2*pi)
        x = np.sin(t*x)
        
        # borwnian motion
        # x[0] = sin(x[0])
        # x[1:] = np.sin(x[1:]+np.exp(x[0]))
