...
def hessian(x: NDArray, f, points_3d, points_2d, K):
    """
    This function computes the Hessian of the objective function.
    """
    # Compute the Hessian of the objective function
    H = scipy.optimize.approx_fprime(x, lambda x: scipy.optimize.approx_fprime(x, f, 1e-8, points_3d, points_2d, K).flatten(), 1e-8)
    return H
...
def optimize(f, x0, points_3d, points_2d, K):
    ...
    # H: NDArray = scipy.optimize.approx_fprime(x, g_, 1e-8)
    H: NDArray = hessian(x, f, points_3d, points_2d, K)
