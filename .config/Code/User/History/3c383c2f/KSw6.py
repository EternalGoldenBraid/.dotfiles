...
class Experiment:
                ...
                if epoch % config.logging.val_print_interval == 0:
                    with torch.no_grad():
                        ...
                        if val_accu > best_val_accu:
                            ...
                            wandb.log({"val": {"best_val_accu": best_val_accu}}, step=epoch, commit=False)
    ...
    def train_end_to_end_gat_inductive(self, config, run_id):
        np.random.seed(config.seed)
        torch.manual_seed(config.seed)
        ...
        # fig_probs.set_facecolor('black')
        # axs_probs[0].set_facecolor('black')
        # axs_probs[0].set_aspect('equal', 'box')
        # axs_probs[0].axis('off')
        ...
        attention_save_dir = Path(config.visualize_dir, 'attention',run_id,'val','images')
        ...
        for trial in range(args.ntrials):
            ...
            plotting_intervals: List[int]  = exp_plotting_intervals(n=args.epochs+1, start=2, end=25)
            for epoch in tqdm(range(1, args.epochs + 1), desc='Epoch: ', position=0, leave=True):
                train_loss = 0.0
                train_accu = 0.0
                val_loss = 0.0
                ...
                if epoch == 1 or epoch % plotting_intervals[epoch] == 0:
                    ...
                    with torch.no_grad():
                        ...
                        plot_probs(dataset=dataset, G=dataset.G, att_weights=self.att_weights.detach().cpu(),
                                ...
                                axs=axs_probs, fig=fig_probs, title=None, 
                                epoch=epoch, save_dir=attention_save_dir, config=config,
                                cmap=cmapper,
                                # cmap=cmap_probs
                                )
                        ...
                        if val_accu > best_val_accu or val_loss < best_val_loss:
                            ...
                            # Store evaluation metrics
                            wandb.log({"val": {"best_val_accu": best_val_accu}}, commit=False, step=epoch)
        ...
        wandb.log({
            ...
            f"{path.split('/')[-1].split('.')[0]}": wandb.Video(path, fps=4, format="mp4") for i, path in enumerate(video_save_paths)
            }, commit=True)
...
def main(config):
    ...
    if config.model.name == "end2end":
        ...
        experiment.train_end_to_end_gat_inductive(config, run_id=run.id)
