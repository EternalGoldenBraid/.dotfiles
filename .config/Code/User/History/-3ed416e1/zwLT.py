...
# translation_time = 10  # 40 minutes. Always specify as milliseconds
...
ribosome_all_positions = np.zeros((2, translation_time), dtype = int)
...
for x in range(translation_time):   # milliseconds. Variable defined above
    ...
    ribosome_all_positions[0, x] = ribosome1_position
    ribosome_all_positions[1, x] = ribosome2_position
...
from visualize import get_positions_at_time_t, update_plot

# Set up plot
fig, ax = plt.subplots()
total_time_steps = ribosome_all_positions.shape[0]

# Create animation
ani = FuncAnimation(fig, update_plot, frames=range(-1, total_time_steps),
                    fargs=(ax, ribosome_all_positions.T), interval=1000, repeat=False)
