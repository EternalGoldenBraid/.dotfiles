...
class VaalikoneDataset(Dataset):
    ...
    def __init__(self, path: Path, config: Dict, tokenizer=None):
        ...
        # (question_id, question, label, comment)
        self.data_new = pd.DataFrame(columns=["question_id", "question", "label", "comment"])
        for i in range(1, 6):
            question_id = f"Lt_{i}"
            question = self.data[question_id].iloc[0]
            label = f"Ln_{i}"
            comment = self.data[label]
            self.data = self.data.append(pd.DataFrame({"question_id": question_id,
                                                       "question": question,
                                                       "label": label,
                                                       "comment": comment}))
        ...
        # self.data.iloc[:, 5:] = self.data.iloc[:, 5:].replace("-", "Tyhjä")
        # self.data = self.data[self.data.apply(lambda x: x.str.len().gt(0).all(), axis=1)]
        
        # Drop rows where "-" is in the first 5 columns # NOTE HACK DEPENDS ON FIRST COLUMN
        # self.data = self.data[self.data[self.data.columns[:5]] != "-"]
        self.data = self.data[(self.data[self.data.columns[:5]] != "-")[self.data.columns[0]]]
        ...
        self.data = self.data[self.data[self.data.columns[5:]].apply(lambda x: x.str.len().gt(0).all(), axis=1)]
