...
from utils.utils import (EarlyStopper, aggregate_statistics, aggregate_from_disk, save_user_feature, CustomSampler)
...
class Experiment:
    ...
    def train_end_to_end_mlp(self, config):
        ...
        dataset = VaalikoneDataset(config=config, debug=config.debug, tokenizer=tokenizer)
        
        users_data = dataset.create_users_data()

        bert_input = config.bert["max_token_length"] * 2
        
        # Get train/val/test splits
        splits = dataset.get_idx_split()
        
        # Create custom samplers for each split
        train_sampler = CustomSampler(users_data, bert_input, indices=splits["train"])
        val_sampler = CustomSampler(users_data, bert_input, indices=splits["val"])
        test_sampler = CustomSampler(users_data, bert_input, indices=splits["test"])
        
        # Create DataLoaders for each split
        train_dataloader = torch.utils.data.DataLoader(dataset, batch_size=2, shuffle=False, sampler=train_sampler)
        val_dataloader = torch.utils.data.DataLoader(dataset, batch_size=2, shuffle=False, sampler=val_sampler)
        test_dataloader = torch.utils.data.DataLoader(dataset, batch_size=2, shuffle=False, sampler=test_sampler)
