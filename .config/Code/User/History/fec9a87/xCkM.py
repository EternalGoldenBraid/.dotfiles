...
def get_event_tag_data(engine, timespan=0, freq_threshold=2,
    ...
    # Map tag names to tag id's and add to data
    tag_names = tags.loc[tag_id_columns]['tag_name']
    tag_names.index.names=['tag_id']
    data = data.merge(tag_names,how='inner',left_on='tag_id',right_index=True)
    data = data.rename(columns={'tag_name':'tag'})
