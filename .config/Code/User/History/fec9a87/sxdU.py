...
def get_event_tag_data(engine, timespan=0, freq_threshold=2,
    ...
    data = data.merge(stories,how='inner', left_index=True, right_on='event_id')
    ...
    # data['tag_id'] colums contains lists of tag_id's
    # for each event_id. We want to create a new column
    # for each tag_id, and fill it with the tag_name.
    for tag_id in tag_id_columns:
        data[tag_id] = data['tag_id'].apply(lambda x: tag_names[tag_id] if tag_id in x else None)

        
    
    #features = ['tag_id', 'meditation', 'rating_sleep', 'rating_day', 'cw', 'screen']
    features = ['tag_id', 'meditation', 'rating_sleep', 'rating_day', 'cw', 'screen', 'date', 'story']
    ...
    # tag_names = tags.loc[event_features['tag_id'].values
    tag_names = tags.loc[tag_id_columns]['tag_name']
