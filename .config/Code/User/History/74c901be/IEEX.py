...
from src.utils import plot_grad_flow, plot_probs, backward_hook, hook
...
class Experiment:
    ...
    def get_loss_gat_inductive(self, model, batch, batch_size=1):
        ...
        if batch_size == 1: # Save att for val
            
            self.att_weights[batch.graph_init_id] = att_weights.detach().squeeze()
            batch.att_weights = att_weights.detach().squeeze()

        return loss, accu
     
    def get_loss_gat_inductive_(self, model, dataloader):
        total_loss = 0
        total_accu = 0
        ...
        for graph_idx, data in enumerate(dataloader):
            ...
            # loss = F.nll_loss(logp, data.y, reduction='mean')
            loss = F.binary_cross_entropy_with_logits(logits, data.y, reduction='mean', 
                    # pos_weight=data.pos_weight
                    )
            ...
            if dataloader.batch_size == 1: # Save att for val
                
                self.att_weights[data.graph_init_id] = att_weights.detach().squeeze()
                data.att_weights = att_weights.detach().squeeze()
    ...
    def train_end_to_end_gat_inductive(self, config):
        ...
        # Clear plot dir
        for f in glob.glob('outputs/grads/*'):
            os.remove(f)
        # fig_gradients.tight_layout()
        fig_gradients.suptitle('Gradients')
        ...
        self.att_weights = torch.zeros(
                                (dataset.num_inits, dataset.edge_index_shape[1]),
        ...
        for trial in range(args.ntrials):
            model = GAT(in_channels=nfeats, hidden_channels=args.hidden, out_channels=nclasses,
                           ...
                           concat=args.concat_multihead, device=self.device, softmax=config.model.softmax)
            ...
            for epoch in tqdm(range(1, args.epochs + 1), desc='Epoch: ', position=0, leave=True):
                ...
                train_loss, train_accu = self.get_loss_gat_inductive(model, dataloader=train_loader)
                train_loss.backward()
                plot_grad_flow(model.named_parameters(), fig=fig_gradients, ax=ax_gradients)
                # plt.show()
                ...
                if epoch % config.train_print_interval == 0:
                    print(f"Epoch {epoch:05d} | Train Loss {train_loss.item():.4f} | Train Accuracy {train_accu:.4f}")
                    # plt.plot(self.att_weights[0].numpy())
                    # plt.show()
                    # attentions = np.concatenate(axis=0,
                    #                 (self.att_weights[0].detach().cpu().numpy(),
                    #                 dataset.data_train.edge_index.detach().cpu().numpy()))
                    
                    # DEBUG
                    # data = dataset.data_train[0]
                    # transmission_matrix = torch.zeros(data.x.shape[0], data.x.shape[0])
                    # t_rates = data.transmission_rate
                    # index = data.edge_index[0]
                    # t_rates_norm = tg.utils.softmax(src=t_rates, index=index)
                    # t_rates_norm = dataset.t_rates_norm
                    # transmission_matrix = transmission_matrix[edge_index_[0] != edge_index_[1], :][:, edge_index_[0] != edge_index_[1]]
                    # print('-----------------')
                    # att_w = self.att_weights
                    # att_w = dataset.data_train[0].att_weights
                    # attentions = np.stack(
                    #                 (att_w,
                    #                 # dataset.data_train[0].transmission_rate
                    #                 t_rates_norm 
                    #                 ),
                    #                 axis=1)
                    
                    # d = {f"similarity_{k}": v.item() for (k, v) in enumerate(similarity)}
                    # wandb.log({"train": d }, commit=True)
                    # wandb.log(
                    # visualizer_attention.update(frame=similarity)
                    # d = {f"similarity_{k}": v.item() for (k, v) in enumerate(similarity)}
                    # wandb.log({"train": {"max_similarity": similarity.max().item()}}, commit=False)
                    # wandb.log({"train": d }, commit=True)
                ...
                if epoch % config.val_print_interval == 0:
                    with torch.no_grad():
                        ...
                        val_loss, val_accu = self.get_loss_gat_inductive(model, dataloader=val_loader)

                        # DEBUG
                        logits, edge_index_, att_weights_ = model(
                                    dataset.data_train[0].x,
                                    dataset.data_train[0].edge_index,
                                    )
                        probs = F.softmax(logits, dim=1)
                        ax_probs.cla()
                        title = f'Probabilities, epoch {epoch}, loss {train_loss.item():.4f}, acc {train_accu:.4f}'
                        ...
                        plot_probs(
                                #    probs=probs[:,1].detach().cpu().numpy(), 
                                   probs=(probs if probs.ndim == 1 else probs[:,1]).detach().cpu().numpy(), 
                                #    gt=dataset.data_train[0].y.detach().cpu().numpy(),
                                   gt=(dataset.data_train[0].y if dataset.data_train[0].y.ndim == 1
                                        else dataset.data_train[0].y[:,1]).detach().cpu().numpy(),
                                   ax=ax_probs, title=title)
                        fig_probs.savefig("probs.png")
