    ...
    with torch.no_grad():
        ...
        if dataset is None:
            ...
            for batch_idx in idxs:
                ...
                input_ids, labels = collator.torch_mask_tokens(input_ids, cfg.data.dataset.mask_probability)
                attention_mask = (input_ids != model.tokenizer.pad_token_id).long()
                input_ids = input_ids.to(device)
                attention_mask = attention_mask.to(device)
                labels = labels.to(device)
                ...
                predicted_token_ids = outputs.logits.argmax(dim=-1)
                predicted_text = model.tokenizer.batch_decode(predicted_token_ids, skip_special_tokens=True)
