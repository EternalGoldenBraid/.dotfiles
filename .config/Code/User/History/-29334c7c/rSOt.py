...
def evaluate_mlm(model,  cfg, dataloader=None, dataset=None, device='cuda', max_samples=10,
    ...
    # target_column = ['story']
    target_column = ['question', 'comment']
    ...
    with torch.no_grad():
        ...
        if dataset is None:
            ...
            for batch_idx in idxs:
                ...
                #     print("Predicted:", predicted_text)
                #     print(30 * "-")
                text = [*dataset.df.iloc[batch_idx][target_column]]
                log_entry = {
                    f'test_{batch_idx}_text': text,
                ...
                print("STORY TOKENS:")
                print(model.tokenizer.convert_ids_to_tokens(model.tokenizer.encode(*text)))
                print()
                
                print("ACTUAL:")
                print(model.tokenizer.convert_ids_to_tokens(input_ids[0]))
                print()
                print("PREDICTED:")
                print(model.tokenizer.convert_ids_to_tokens(predicted_token_ids[0]))
                print(30 * "-")
