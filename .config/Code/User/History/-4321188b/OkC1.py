...
class MLMVaaliKoneDataset(Dataset):
    def __init__(self, root, config, filename, tokenizer: PreTrainedTokenizer):
        ...
        self.df = pd.read_csv(os.path.join(root, 'raw', filename))
        # self.df = self.df.iloc[-10:]
        # self.df = self.df.iloc[-20:]
        # self.df = self.df.iloc[-1000*config.model.language_encoders.perttu.batch_size:]
        # self.df = self.df.iloc[-5*config.model.language_encoders.perttu.batch_size:]
        # self.df = self.df.sample(5*config.model.language_encoders.perttu.batch_size)

    def __len__(self):
        return len(self.df)

    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor]:
        comment = self.df.iloc[index]['comment']
        question = self.df.iloc[index]['question']
        input_ids = self.tokenizer.encode(question, comment,
                                        add_special_tokens=True, return_tensors='pt').squeeze()

        return input_ids

        # Datacollator takes care of masking.
        mask = torch.rand(input_ids.size(0)) < self.config.data.dataset.mask_probability
...
class VaalikoneDataset(Dataset):
    ...
    def __init__(self, config: Dict, tokenizer=None, debug=False):
        """
        ...
        self.data: pd.DataFrame = pd.read_csv(path, index_col=None)
    ...
    def __getitem__(self, idx):
        """
        Get the questions and comments of a participant at idx.
        Tokenize the questions and comments.
        Create the attention mask and token type ids.

        Args:
            idx: Index of the participant to be retrieved.

        Returns:
            sample Dict: Keyed by the question id. 
                Contains all tokenized (question,answer) pairs of a participant at idx.

        TODO 1: Add a split token between question and comment.
        TODO 2: Add a special token for the question and comment?
        TODO 3: Add a token or learn a token for region.
        """
        
        # Index should be participant_id
        participant = self.participants.index[int(idx)]
        
        # print("Number of questions:", len(self.data[self.data["participant_id"] == participant]))
        
        participant_data = self.data[self.data["participant_id"] == participant]
        
        sample: Dict = self.tokenize_participant_data_(participant_data)
        sample['participant_idx'] = idx
