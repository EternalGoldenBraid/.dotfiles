...
class Experiment:
    ...
    def train_end_to_end_mlp(self, config):
        ...
        dataset, nclasses, train_mask, val_mask, test_mask = load_vaaliperttu_data(config, 
                                                                tokenizer=tokenizer)
        model_bert.init_outputs(dataset.max_questions)
        bert_outputs_dir = Path(config.paths.outputs_dir, 'bert_outputs')
        
        loader = torch.utils.data.DataLoader(dataset, batch_size=80, shuffle=True)
        ...
        with Pool(processes=5) as pool:
            ...
            for epoch in tqdm(range(1, args.epochs_adj + 1), leave=False, desc="Epochs"):
                model_bert.train()
                model.train()
                ...
                # model_bert.bert_step(dataloader=loader, config=config, features=features)
                
                accumulated_features = torch.empty_like(features)
                ...
                for data in tqdm(loader):
                    ...
                    ### DEBUG
                    # input_ids = torch.rand_like(input_ids, dtype=torch.float).to(self.device)
                    # attention_mask = torch.randint_like(input_ids, 1).to(self.device).bool()
                    ### END DEBUG
                    
                    # features[features_idxs] = model_bert(input_ids=input_ids, 
                    ...
                    # features[features_idxs] = updated_features.clone()
                    # accumulated_features[features_idxs] = bert_features
                # features = accumulated_features.clone()
                # features = accumulated_features
                                                    
                    ### DEBUG
                    # features[features_idxs] = torch.rand_like(features[features_idxs], dtype=torch.float).to(self.device)
                    # features[features_idxs] = features[features_idxs].detach()
                    ### END DEBUG
                                                                
                # participant_features = aggregate_statistics(
                # participant_features = aggregate_statistics(
                #                     participant_features=participant_features, 
                #                     features=features, 
                #                     participant_ids=participant_ids, 
                #                     participant_idx_to_id=participant_feature_mask)
                
                participant_features = pool.map(aggregate_from_disk, zip(dataset.participants.participant_id.values, [bert_outputs_dir] * dataset.n_participants))

                ### DEBUG
                # participant_features = torch.rand_like(participant_features, dtype=torch.float).to(self.device)
                # participant_features = features[:participant_features.shape[0]].detach()
                # participant_features = features[:dataset.n_participants].clone()
                # logits = model(participant_features)
                # logits = model(features[:dataset.n_participants])
                # logits = model(features_)
                # logp = F.log_softmax(logits, 1)
                # loss = F.nll_loss(logp[train_mask], labels[train_mask], reduction='mean')
                # accu = accuracy(logp[mask], labels[mask])
                ### END DEBUG

                loss, accu = self.get_loss_fixed_adj(model, train_mask, participant_features, labels)
