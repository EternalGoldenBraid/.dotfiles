...
def plot_probs(dataset, G: nx.Graph, att_weights: torch.tensor, sources: list[list[str]], probs: torch.tensor,
                edge_index: torch.tensor, axs: plt.axes, fig: plt.figure, title: str, save_dir: str, cmap :plt.cm,
                epoch: int = 0):
    ...
    for i in range(att_weights.shape[0]):
        ...
        title = (f"Sources: {sources[i]}, \
                ...
                Pearson corr: {pears_corr[i].correlation.2f}, p-value: {pears_corr[i].pvalue:.2f} \
                ")
        ...
        wandb.log({"val": {"pears_corr": pears_corr[i].correlation, 
                    "kendall_tau": kendall_tau[i].correlation,
                    "pears_corr_pval": pears_corr[i].pvalue,
                    "kendall_tau_pval": kendall_tau[i].pvalue,
                    "pears_corr_abs": np.abs(pears_corr[i].correlation),
                    }})
