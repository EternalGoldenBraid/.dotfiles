...
def plot_probs(config, dataset, G: nx.Graph, att_weights: torch.tensor, sources: list[list[str]], probs: torch.tensor,
    ...
    # edge_color = cmap(norm(distance))
    # edge_color = cmap(distance)
    # edge_color = cmap(edge_attr.int()*255)
    edge_color = cmap(edge_attr)
    # print(edge_color)
    # edge_color = np.ones((edge_attr.shape[0], 4))
...
def make_probs_movie_old(config, path: Path):
    """
...
def make_probs_movie(config, path: Path) -> List[str]:
    """
    Make a movie for each initialization of the figures stored in path.
    ...
    def find_files(init_infection: str):
        search_pattern = str(path) + "/probs_" + str(init_infection)
        all_files = glob.glob(str(path) + f'/*{init_infection}*.png')
        filenames = sorted([f for f in all_files if search_pattern in f], key=lambda x: int(''.join(filter(str.isdigit, x))))
        for filename in filenames:
            yield filename

    # Make movie for each initial infection
    init_infections = list(set([f.split('_')[1] for f in glob.glob(str(path) + '/*') if f.endswith('.png')]))

    out_paths = []

    for init_infection in init_infections:
        print(f"Making movie for {init_infection},", end=" ")
        ...
        # Make mp4 movie with mediapy with the filenames in filenames
        filenames = find_files(init_infection)
        images = (mediapy.read_image(f) for f in filenames)
        # RGBA to RGB
        images = (img[:,:,:3] for img in images)
        mediapy.write_video(movie_out, images, fps=10)

        out_paths.append(movie_out)
        print(f"Movie saved to {movie_out}")

    return out_paths


def make_probs_movie_old2(config, path: Path):
    """
    Make a movie for each initialization of the figures stored in path.
