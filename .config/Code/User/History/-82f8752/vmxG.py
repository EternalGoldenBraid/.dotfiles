...
def animate(w, time: float, pcl=object_pcl):
    ...
    R = o3d.geometry.get_rotation_matrix_from_quaternion(
        rotation=[watch.quaternion[-1], *watch.quaternion[:-1]]).as_matrix()
    print(f"Watch quat: {watch.quaternion}")
    print(f"rot: {sp_R.from_quat(watch.quaternion).as_matrix()}")
    print(f"Current frame: {sp_R.from_quat(watch.quaternion).as_matrix()}")

    # sleep(1)
    # print([watch.quaternion[-1], *watch.quaternion[:-1]])
    R = sp_R.from_quat([watch.quaternion[-1], *watch.quaternion[:-1]]).as_matrix()
    print("Rot idx:", rot_idx)
    print(np.abs(R@np.linalg.inv(R_prev) - id))
    atol = 1e-9
    print(np.allclose(R @ np.linalg.inv(R_prev), id, atol=atol))
