...
def aggregate_from_disk(config: DictConfig) -> torch.tensor:
    """
        Aggregate the features of the participants in the batch.

    """
    user_dir = f"bert_features/user_{user_id}"
    feature_files = os.listdir(user_dir)
    user_features = [torch.load(f"{user_dir}/{file}") for file in feature_files]
